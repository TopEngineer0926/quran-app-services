...
class Resource extends Model
{
    ...
    {
        Source::destroy($this->source()->first()->id);
        parent::delete();
    }
    /**
    ...
    public function getIsAvailableAttribute($data)
    {
        if($data == 1)
        {
            return '<a style="color:red" title ="Click To Deactivate" href = "'.route('vbv.deactivate',['id'=>$this->attributes['id']]).'">Deactivate</a>';
        }
        else
        {
            return '<a style="color:green" title ="Click To Activate" href = "'.route('vbv.activate',['id'=>$this->attributes['id']]).'">Activate</a>';
        }
    }
}
