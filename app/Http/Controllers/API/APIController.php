...
class APIController extends Controller
{
    ...
    protected function chapters(Request $request)
    {
        ...
        if ($language) {
            ...
            if(empty($translated_names)){
                $lang_eng = Languages::where('iso_code', 'en')->first();
                $translated_names = TranslatedName::select('language_name', 'name')->where('language_id', $lang_eng->id)->where('resource_type', 'chapters')->get()->toArray();
            }
            $name = TranslatedName::select('language_name', 'name')->where('language_id', $language_id)->where('resource_type', 'chapters_name')->get()->toArray();
            $chapters = Chapter::get();
            $loop = 0;
            if ($chapters) {
                foreach ($chapters as $chapter) {
                    $chapter->pages = json_decode($chapter->pages);
                    if(isset($translated_names[$loop])){
                        $chapter->setAttribute('translated_name', $translated_names[$loop]);
                    }
                    else
                    {
                        $chapter->setAttribute('translated_name', ['language_name' => lcfirst($language->name),'name' => $chapter->name_complex]);
                    }
                    if(isset($name[$loop])){
                        $chapter->name_simple = $name[$loop]['name'];
                    }
                    $loop++;
                }
            }
    ...
    protected function verses($id, Request $request)
    {
        ...
        $verses = Verses::select(
            ...
            ->with(['words.translation' => function ($q) use (&$language) {
    ...
    protected function suggest(Request $request)
    {
        ...
        $sql = "SELECT
                    id,text_madani,text_indopak,text_simple,verse_key,verse_number,chapter_id,MATCH (text_madani) AGAINST (
                        '$query' IN NATURAL LANGUAGE MODE
                    ) AS rank
                    FROM
                        verses
        ...
        foreach ($records as $record) {
            ...
            $suggest->put('chapter_id', $record->chapter_id);
            $suggest->put('verse_number', $record->verse_number);
            $suggest->put('translation_id', null);
        ...
        if (count($suggests) < $limit) {
            ...
            foreach ($records as $record) {
                ...
                $suggest->put('chapter_id', $verse->chapter_id);
                $suggest->put('verse_number', $verse->verse_number);
                $suggest->put('translation_id', $record->resource_id);
