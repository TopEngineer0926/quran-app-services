...
class ImportController extends Controller // commented to provent any accidental executions
{
    ...
    //             if($url){
    //             $url = explode('/',$url);
    //             $word->audio_url = 'wordbyword'.'/'.$url[1];
    //             $word->save();
    //             }
    //         }

    //     }
    protected function insert_translated_names(Request $request)
    {
        $is_deleted = 'no';
        ...
        $language = Language::where('iso_code', $language_code)->first();
        $file = Storage::disk('public')->get($filename);
        $lines = explode("\n", $file);
        $check_exists = TranslatedName::where('resource_type','chapters')->where('language_id',$language->id)->first();
        if($check_exists)
        {
            TranslatedName::where('resource_type','chapters')->where('language_id',$language->id)->delete();
            $is_deleted = 'yes';
            //return 'translated names already exist please delete from table: iso code:'.$language_code.' language: '.lcfirst($language->name);
        }
        ...
        if ($language) {
            ...
            foreach ($lines as $line) {
                $translated_name = new TranslatedName;
                $translated_name->resource_type = 'chapters';
                $translated_name->resource_id = $resource_id;
                $translated_name->language_id = $language->id;
                $translated_name->name = $line;
                $translated_name->language_name = \lcfirst($language->name);
                $translated_name->language_priority = $priority;
                $translated_name->save();
                $resource_id++;
            }
        }
        return $resource_id-1 . ' translated names inserted. Deleted: '.$is_deleted;
    }

    protected function insert_chapter_names(Request $request)
    {
        $is_deleted = 'no';
        $filename = $request->filename;
        $language_code = $request->language_code;
        ...
        $file = Storage::disk('public')->get($filename);
        $lines = explode("\n", $file);
        $check_exists = TranslatedName::where('resource_type','chapters_name')->where('language_id',$language->id)->first();
        if($check_exists)
        {
            TranslatedName::where('resource_type','chapters_name')->where('language_id',$language->id)->delete();
            $is_deleted = 'yes';
            //return 'translated names already exist please delete from table: iso code:'.$language_code.' language: '.lcfirst($language->name);
        }
        if ($language) {
            ...
            foreach ($lines as $line) {
                $translated_name = new TranslatedName;
                $translated_name->resource_type = 'chapters_name';
                $translated_name->resource_id = $resource_id;
                $translated_name->language_id = $language->id;
                $translated_name->name = $line;
                $translated_name->language_name = \lcfirst($language->name);
